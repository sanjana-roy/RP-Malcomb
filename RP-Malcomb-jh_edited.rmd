...
dhsclusters_2010 = readRDS(here("data", "raw", "private", "datasets", "MWGE62FL.rds" )) %>% #dhs_downloads$MWGE62FL
  as("sf") %>% 
  st_transform(3395) %>% 
...
dhshh_2010 = readRDS(here("data", "raw", "private", "datasets", "MWHR61SV.rds")) %>% zap_labels() #dhs_downloads$MWHR61SV
```
...
#geo is one layer parameter, in this case, we will call the vulnerability 
vulnerability = function(geo) {
  ...
  final = (40 - geo) * 0.40 + drought * 0.20 + flood * 0.20 #add sensitivity * 0.20? 
  return (final)
}
```
